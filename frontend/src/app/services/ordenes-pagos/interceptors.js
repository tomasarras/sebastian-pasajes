...
const axiosIntance = axios.create({
  baseURL: process.env.NEXT_PUBLIC_ORDENES_PAGOS_SERVER_BASE_URL,
})
...
axiosIntance.interceptors.request.use((config) => {
  const token = localStorage.getItem('opAccessToken');
  if (token) {
...
axiosIntance.defaults.baseURL = process.env.NEXT_PUBLIC_ORDENES_PAGOS_SERVER_BASE_URL;
